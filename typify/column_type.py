...
class column_typer:
	...
	def build_classifiers(self):
		...
		sa_regex = r'''^(?:[Oo][Nn][Ee]|[0-9-]*[a-zA-Z]?)\s+(?:[NSEW]\.?[NSEW]?\.?\s+|[NSEWnsew][OAEoae][RUSrus][Tt][Hh]?\s+)?(?:\d*(?:[SNRTsnrt][TDHtdh])?\s+|(?:[a-zA-Z'-]*\s+)*?)(?:[a-zA-Z'-]*)?\.?(?:\s+\d*)?,?(?:\s+[NSEW]\.?[NESW]?\.?|\s+[NSEWnsew][OAEoae][RUSrus][Tt][Hh]?)?(?:\s+(?:[a-zA-Z][a-z]*\.?|[Pp][Oo]\.?\s+?[Bb][Oo][Xx])?(?:\s+[#]?\d*[a-zA-Z]?))?$'''
